...
struct wait_status {
  ...
  bool waited_child; /* can only wait once for each child */
};
...
struct thread {
  ...
  struct wait_status *wait_status; /* Each thread stores its own wait status. */
  size_t fd_count; // Keeps track of the number of open file descriptors for this process 
  struct list file_descriptors; // Holds a mapping to the process' open files. Used to close them
